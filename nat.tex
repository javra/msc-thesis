This section describes the basic notions of non-abelian topology which I
...
line $f \in D_1$. Degenerate lines are drawn as double lines.}
...
\end{tikzpicture}
\caption{Lower right and upper left connection of a morphism $f$.}
...
\begin{lemma}[Coherence of inverses] \label{thm:dbl-gpd-inv}
...
\sectionmark{DGpd and XMod are Equivalent}
...
\begin{tikzpicture}[auto,scale=2,color=black,every path/.append style={thick}]
\draw[double] (0,1) -- (0,0);
\draw[double] (1,1) -- (1,0) -- (2,0);
\draw[double] (3,0) -- (4,0) -- (4,1);
\draw (0,1) -- (1,1) node [midway,above] {$g\inv$};
\draw (1,1) -- (2,1) node [midway,above] {$h\inv$};
\draw (2,1) -- (3,1) node [midway,above] {$f$};
\draw (3,1) -- (4,1) node [midway,above] {$i$};
\draw (0,0) -- (1,0) node [midway,below] {$g\inv$};
\draw (2,1) -- (2,0) -- (3,0) node [midway,below] {$g$} -- (3,1);
\foreach \x in {0,1,2,3,4} {
	\foreach \y in {0,1} {
		\node[fill,circle,inner sep=0pt,minimum size=5pt] at (\x,\y) {};
	}
}
\node at (0.5,0.5) {\footnotesize{$\invh\id_1(g)$}};
\node at (1.5,0.5) {\footnotesize{$\invh\Gamma^-(h)$}};
\node at (2.5,0.5) {$u$};
\node at (3.5,0.5) {$\Gamma^-(i)$};
\end{tikzpicture}
\caption{Folding the faces of a square $u$ to its upper face.}
\label{fig:fold}
\end{figure}
