...
double groupoid of a triple of spaces. It extends and generalizes the definition
of the fundamental group and the second homotopy group of a space.

\begin{defn} \label{def:filtered-dbl-gpd}
Let $C \subseteq A \subseteq X$ be a triple of topological spaces. The
\textbf{filtered map double groupoid} $R(X, A, C)$ of $X$, $A$ and $C$ is the
double groupoid with
\begin{itemize}
\item $R(X, A, C)_0 = C$,
\item $R(X, A, C)_1 = \left\{ \sigma : (I, \partial I) \to (A, C) \right\}$ and
\item $R(X, A, C)_2 = \left\{ \alpha : (I^2, \partial I^2, \partial^2 I^2)
	\to (X, A, C) \right\}$.
\end{itemize}
...
To make this a double groupoid we have to state faces, degeneracies, compositions
and inversions. They might be well known to the reader who is familiar with homotopy
since they are just an extension of the operation that definies the loop group in
